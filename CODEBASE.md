...
| `frontend/lib/supabase.ts` | Module-level browser Supabase client — `createBrowserClient(NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY)`; exported as default singleton; imported by all client components that need auth or data |
...
| `frontend/.env.local` | Sets `NEXT_PUBLIC_API_URL=http://localhost:8000`, `NEXT_PUBLIC_SUPABASE_URL`, and `NEXT_PUBLIC_SUPABASE_ANON_KEY` |
