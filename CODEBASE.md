...
| `frontend/app/first-post/page.tsx` | Standalone conversion-focused route — no sidebar, no AppShell, handles its own auth check. Shows Screen 0 (auth invite card) if unauthenticated (Google OAuth via `useSignIn().authenticateWithRedirect`, email link to `/sign-up?redirect_url=/first-post`). For signed-in users: 5 sequential screens (role picker → topic+format → experience type → opinion picker → audience+voice) with progress dots, fade transition, Back/Next nav, and per-screen validation. All collected data stored in a single `answers` state object. Screen 5 "Generate" button calls `handleGenerate()` which for now `console.log`s answers and shows a generating placeholder screen. Uses `useUser()` for auth gate; no backend calls in this file. |
