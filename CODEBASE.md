...
| `backend/tests/test_library.py` | `DELETE /library/source` removes chunks. Two strict xfails document a bug: the router treats `delete_source()`'s dict return as a count (no 404 for missing sources; `chunks_removed` holds the whole dict) |
