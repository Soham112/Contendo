...
| `extension/popup.html` | Extension popup UI — states: Connecting (spinner, shown while resolving token); Connect (shown when Contendo is not open — single "Open Contendo" button); Page (title preview + Article/Personal note pills + Add to Memory); YouTube (title preview + transcript textarea + Add video to Memory); Loading; Success. Token input and manual paste flow removed entirely. Styled with Contendo off-white/sage palette, no build step. |
| `extension/popup.js` | Popup controller — auto-auth flow: (1) check `chrome.storage.local` for cached token; (2) if none, query for open `contendo-six.vercel.app` tab via `chrome.tabs.query`, send `{action:"getSupabaseToken"}` to its content script, cache result; (3) if token still unavailable, show Connect state with "Open Contendo" button; (4) on 401 response from backend, clear cached token and re-run flow for automatic re-auth. No manual token input or "Change token" link anywhere. |
...
| `extension/background.js` | MV3 service worker — listens for `{action:"ingest"}` messages; calls `POST /ingest` with `Authorization: Bearer {token}` (mirrors `useApi()` auth pattern); returns `{success, data}` on success or `{success:false, error, status}` on failure — `status` field enables popup to detect 401 and trigger automatic re-auth; sets sage-green ✓ badge for 3 s on success |
| **Frontend** | |
