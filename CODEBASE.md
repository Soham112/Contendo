...
| `frontend/components/AppShell.tsx` | Layout wrapper — renders Sidebar + FeedbackButton for app routes; returns children unwrapped for `/welcome`, `/onboarding`, `/first-post`, `/sign-in`, `/sign-up`; runs `useProfileCheck()` (which is a no-op on `/first-post`) and shows a loading spinner while profile check is in flight; **intercept safety net:** after profile check confirms `has_profile === true`, checks `localStorage.contendo_intercept_done` — if absent, renders `OnboardingIntercept` with `destination=pathname` before showing the workspace; `onComplete` callback sets local `interceptDone` state to true so the app shell re-renders with the full workspace immediately |
| `frontend/components/OnboardingIntercept.tsx` | One-time 5-question intercept shown after the first-post flow (and as a safety net for returning users who missed it). Full-screen white card (`rounded-2xl`, ambient shadow). Five questions shown one at a time with progress dots (sage fill = seen, ghost border = upcoming). Q1–Q4 are chip-select (single selection per question); Q5 is a free-text input. **Question → profile field mapping:** Q1 (`voice_descriptors`): communication style chip; Q2 (`voice_descriptors`): writing mode chip; Q3 (`target_audience`): audience scalar — only set if currently empty; Q4 (`writing_rules`): goal chip transformed as `"My goal: {chip}"`; Q5 (`opinions`): free text appended if non-empty. **Save logic:** loads existing profile via `GET /profile`, merges answers (list fields appended with case-insensitive dedup, scalar only set if empty), saves via `api.saveProfile()`. **Completion:** sets `localStorage.contendo_intercept_done = "1"`, calls `onComplete?.()`, navigates to `destination` prop. **Skip:** any question can be skipped; last-question skip still triggers save + navigation. Skipping all questions still sets the localStorage key so the intercept never shows again. |
