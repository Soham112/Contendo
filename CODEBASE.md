...
| `backend/requirements.txt` | All Python dependencies pinned — includes rank-bm25>=0.2.2 for hybrid BM25 retrieval, supabase>=2.0.0, asyncpg>=0.29.0, youtube-transcript-api>=0.6.0 (MIT license, no API key) |
| `backend/.env.example` | Required env var template — `ANTHROPIC_API_KEY`, `DATA_DIR`, `FRONTEND_ORIGIN`, `ENVIRONMENT`, `SUPABASE_JWT_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ADMIN_SECRET` |
