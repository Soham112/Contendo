...
| `frontend/app/welcome/page.tsx` | Landing page (`/welcome`) — standalone editorial marketing surface (top nav + hero + testimonial band + philosophy split + toolkit cards + inspiration strip + final CTA + footer), accessible to both signed-in and signed-out users. Includes a functional hero prompt bar: submit on Enter or button click; empty submit shows subtle inline validation; submit always writes `sessionStorage.contendo_topic` for continuity; signed-in users are routed to `/create` with topic prefilled via `sessionStorage.contentOS_last_topic`; signed-out users are routed to `/first-post?topic=...` and first-post hydrates from query + `sessionStorage.contendo_topic` fallback so topic survives auth transitions. Auth-aware nav/CTAs remain preserved (`/sign-in?redirect_url=/welcome` for signed-out, "Open workspace" for signed-in). |
...
| Landing page is the default entry point — middleware handles all routing | Three rules in `frontend/middleware.ts`: (1) unauthenticated `/` → `/welcome`; (2) signed-in `/welcome` → `/`; (3) all other protected routes without auth → `/sign-in` with `redirect_url`. The root route (`/`) itself is unchanged — it still renders Feed Memory for authenticated users. The redirect logic lives entirely in middleware so `app/page.tsx` has no auth guards. |
