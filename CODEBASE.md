...
| `frontend/app/welcome/page.tsx` | Landing page (`/welcome`) — accessible to both signed-in and signed-out users; client component using `useUser()` for auth-aware CTAs; editorial atelier marketing page; fixed glassmorphic nav with `font-headline italic` logo + CSS-only dropdowns; signed-out nav: "Log in" → `/sign-in?redirect_url=/welcome` (preserves return destination), "Get started" → `/sign-up`; signed-in nav: single "Open workspace" → `/`; hero and final CTAs follow same auth-aware pattern ("Start writing for free" for signed-out, "Open workspace" for signed-in); hero with Noto Serif h1, feature cards, philosophy section, bento grid, `MOOD_GRADIENTS` staggered mood grid (8 tiles, charcode-deterministic), final CTA with dot-grid overlay; own top nav, AppShell sidebar suppressed |
