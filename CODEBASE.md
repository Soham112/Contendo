...
| **Extension** | |
| `extension/manifest.json` | Manifest V3 Chrome extension declaration — permissions: `activeTab`, `storage`, `scripting`; host_permissions: `https://contendo-production.up.railway.app/*`; background service worker: `background.js`; popup: `popup.html` |
| `extension/popup.html` | Extension popup UI — three states: (1) Auth: token paste form + Save button; (2) Page: extracted title preview, source-type pills (Article / Personal note), Add to Memory button; (3) YouTube: video title preview, transcript textarea, Add video to Memory button. Also includes Loading and Success states. Styled with Contendo's off-white/sage palette, no build step. |
| `extension/popup.js` | Popup controller — on load reads `chrome.storage.local` for token; if missing shows auth state; queries active tab URL to detect `youtube.com/watch`; calls `chrome.scripting.executeScript` to inject `content.js` then `chrome.tabs.sendMessage` to get `{title, text, url}`; on ingest click sends `{action:"ingest", token, tabId, content, source_type, content_origin, title, url}` message to `background.js`; handles success/error display |
| `extension/content.js` | Content script injected on demand via `chrome.scripting.executeScript` — responds to `{action:"getPageContent"}` messages with `{title: document.title, text: document.body.innerText.slice(0,8000), url: location.href}`; injection guard (`window.__contendo_injected`) prevents duplicate listeners on repeated popup opens |
| `extension/background.js` | MV3 service worker — listens for `{action:"ingest"}` messages from popup; calls `POST /ingest` on `contendo-production.up.railway.app` with `Authorization: Bearer {token}` header (same pattern as `frontend/lib/api.ts useApi()`); on success sets a sage-green ✓ badge on the tab icon for 3 s; returns `{success, data}` or `{success:false, error}` to popup |
| **Frontend** | |
