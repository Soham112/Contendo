...
| `backend/agents/draft_agent.py` | Calls Claude Haiku via `infer_archetype()` to classify the topic into one of 7 post archetypes, then generates the initial draft via Claude Sonnet; uses `_format_retrieval_context()` which prefers the enriched `retrieved_context` from state and falls back to flat `retrieved_chunks`; passes `state.get("length", "standard")` into `get_format_instructions()` |
...
| `backend/pipeline/state.py` | TypedDict schema for shared LangGraph pipeline state; includes `length` (`"concise" | "standard" | "long-form"`), `archetype`, `user_id`, `critic_brief`, `retrieved_chunks` (backward compat flat list), `retrieval_bundle` (full hierarchical bundle), and `retrieved_context` (pre-formatted enriched text for draft prompt) |
...
| `frontend/components/AppShell.tsx` | Layout wrapper — renders Sidebar + FeedbackButton for app routes; returns children unwrapped for `/welcome`, `/onboarding`, `/first-post`, `/sign-in`, `/sign-up`; runs `useProfileCheck()` (which is a no-op on `/first-post`) and shows a loading spinner while profile check is in flight |
...
| `frontend/app/first-post/page.tsx` | Standalone conversion-focused route — no sidebar; auth invite for signed-out users (Google OAuth + email), plus a 5-step guided flow for signed-in users (topic+format → role → experience type → opinion → audience+voice). It accepts topic prefill from `/welcome` via `?topic=` query param and `sessionStorage.contendo_topic` fallback, reuses that topic in auth redirect links (`redirectUrlComplete` for Google OAuth and `redirect_url` for email sign-up), and shows a subtle "Writing about: ..." metadata line above the onboarding/auth card from Step 2 onward (hidden on Step 1 where the topic input is already present). For continuity, `contendo_topic` is retained through auth transitions and only cleared after the signed-in flow is hydrated with a non-empty topic (prevents topic loss when auth returns without query params). Step 1 format options are limited to LinkedIn Post and Medium Article (Thread removed from this first-post flow). On "Generate my first post →", `handleGenerate()` first attempts `POST /profile` (non-blocking on failure), then calls `POST /generate` with `{ topic, format, tone: "casual", context: buildContext(answers) }`. While waiting, the generating placeholder remains visible. On generate failure, an in-place error state renders with "Go to workspace →" to `/create`. On success, it attempts non-blocking auto-save via `POST /log-post`, captures `post_id` if returned, and transitions to an in-page draft display card (read-only post, optional authenticity score, expectation-setting callout, copy button, and CTA buttons). Before navigating via either CTA, it writes sessionStorage keys: `contentOS_last_post`, `contentOS_last_topic`, `contentOS_last_format`, `contentOS_last_tone`, and `contentOS_current_post_id` (when available), plus `*_meta` keys consumed by CreatePost restoration. |
