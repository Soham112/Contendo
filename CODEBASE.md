...
| `frontend/components/FeedMemory.tsx` | Feed Memory form — tabs (Article/Text, URL, File, YouTube, Image, Note, Obsidian), URL scraping, textarea, file upload with drag-and-drop, image upload, Obsidian vault preview/ingest flow, result display. **Modal tour:** shown only when BOTH conditions are true: (1) `localStorage.getItem('contendo_feed_tour_done')` is null, AND (2) `stats.total_chunks === 0` (fetched from `GET /stats` on mount). This ensures existing users with content never see the tour regardless of their localStorage state. When both conditions are met, the tour starts 800 ms after stats load. Renders a `position: fixed` full-screen overlay (`z-index: 100`, `rgba(0,0,0,0.55)`) with a centered white card (420px wide, sage top border, drop shadow). Card shows: tab name (16px/600) + step indicator, a divider, the tab icon in sage, description text (15px), and Skip/Next→/Done buttons. Tour walks 6 steps (Article → URL → File → YouTube → Image → Note); each step auto-switches the active tab. Clicking the overlay backdrop also dismisses. Completing or skipping sets `contendo_feed_tour_done = "1"` in localStorage. |
