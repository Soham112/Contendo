...
| `frontend/app/settings/page.tsx` | Settings hub at `/settings` — 2-column card grid (1-col mobile); loads profile + stats in parallel; 6 cards: **Profile**, **Voice & Fingerprint**, **Writing Samples**, **Memory**, **Account** (Supabase `user_metadata.avatar_url`/`full_name`/`email`; sign-out via `supabase.auth.signOut()` then router push), **Integrations** ("Soon"); all cards animate-pulse skeleton while loading |
