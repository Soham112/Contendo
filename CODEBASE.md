...
| Claude API usage logged fire-and-forget via `usage_store.py` | `draft_node`, `critic_node`, `humanizer_node`, `word_count_enforcer_node`, `predictability_audit_node`, and `ideation_agent.generate_suggestions()` each call `schedule_usage_event(...)` immediately after the Claude API call returns; `response.usage.input_tokens` and `response.usage.output_tokens` are read directly from the Anthropic SDK response object. These agents run inside `run_in_threadpool` workers (no running loop there), so `schedule_usage_event` hands the write to the server loop captured in lifespan via `asyncio.run_coroutine_threadsafe`. With no loop at all (scripts, some unit tests) it silently skips. `log_usage_event` itself is wrapped in try/except and never raises — failures only produce a WARNING log line. Supabase `usage_events` table must be pre-created via the SQL in the feature spec. |
