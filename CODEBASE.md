...
| `frontend/components/FeedMemory.tsx` | Feed Memory form — tabs (Article/Text, URL, File, YouTube, Image, Note, Obsidian), URL scraping, textarea, file upload with drag-and-drop, image upload, Obsidian vault preview/ingest flow, result display. **Tooltip tour:** on first visit (localStorage key `contendo_feed_tour_done` absent), a floating tooltip card appears 800 ms after mount, anchored below the active tab pill via `position: absolute` inside a `position: relative` tab-bar wrapper. The tour walks through 6 steps (Article → URL → File → YouTube → Image → Note) in order; each step auto-switches the active tab. A CSS border-triangle caret points upward at the active tab. "Skip tour" and "Next →" / "Done" buttons are right-aligned; completing or skipping sets `contendo_feed_tour_done = "1"` in localStorage so the tour never shows again. Clicking a tab while the tour is active advances the tour to that tab's step. The old empty-state method grid (shown when `total_chunks === 0`) has been removed; the tour replaces it as the onboarding mechanism. |
