...
| `frontend/app/welcome/page.tsx` | Landing page (`/welcome`) — standalone editorial marketing surface (top nav + hero + testimonial band + philosophy split + toolkit cards + inspiration strip + final CTA + footer), accessible to both signed-in and signed-out users. Includes a functional hero prompt bar: submit on Enter or button click; empty submit shows subtle inline validation; signed-in users are routed to `/create` with topic prefilled via `sessionStorage.contentOS_last_topic`; signed-out users are routed to `/first-post?topic=...` and also write `sessionStorage.contentOS_first_post_topic` as fallback so topic survives auth transitions. Auth-aware nav/CTAs remain preserved (`/sign-in?redirect_url=/welcome` for signed-out, "Open workspace" for signed-in). |
...
| Welcome hero prompt bar routes into product with prefill | Submitting topic text on `/welcome` sends signed-in users to `/create` with `contentOS_last_topic` set (consumed by `CreatePost.tsx` on mount). Signed-out users are sent to `/first-post?topic=...` and also store `contentOS_first_post_topic` as fallback, then `first-post/page.tsx` hydrates the topic from query/session before showing form or auth invite. |
