...
| `backend/tests/test_endpoint_auth_coverage.py` | Inspects every `APIRoute` and fails if one lacks `Depends(get_user_id_dep)` unless listed in `PUBLIC_ROUTES` (with a reason). `KNOWN_UNPROTECTED` lists current gaps (`/refine`, `/score`, `/generate-visuals`, `/obsidian/preview-zip`) as strict xfails — remove an entry once that route is protected |
