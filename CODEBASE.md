...
| `frontend/app/welcome/page.tsx` | Landing page (`/welcome`) — accessible to both signed-in and signed-out users; client component using `useUser()` for auth-aware CTAs; editorial atelier marketing page; fixed glassmorphic nav with `font-headline italic` logo + CSS-only dropdowns; signed-out nav: "Log in" → `/sign-in?redirect_url=/welcome` (preserves return destination), "Write your first post" → `/first-post`; signed-in nav: single "Open workspace" → `/`; hero primary CTA for signed-out users: "Write your first post →" → `/first-post` with supporting subtext "Takes 2 minutes. No setup required." (13px Inter, 50% opacity); signed-in hero CTA: "Open workspace" → `/`; hero with Noto Serif h1, feature cards, philosophy section, bento grid, `MOOD_GRADIENTS` staggered mood grid (8 tiles, charcode-deterministic), final CTA with dot-grid overlay; own top nav, AppShell sidebar suppressed |
...
| `frontend/app/first-post/page.tsx` | Standalone conversion-focused route — no sidebar, no AppShell, handles its own auth check. Shows auth invite card if unauthenticated (Google OAuth via `useSignIn().authenticateWithRedirect`, email link to `/sign-up?redirect_url=/first-post`). On mount for signed-in users: calls `GET /profile` — if `has_profile === true`, redirects immediately to `/`; otherwise shows the 5-screen flow. Screen order: **Screen 0** Topic+Format ("What do you want to write about?") → **Screen 1** Role picker ("What best describes your work?") → **Screen 2** Experience type → **Screen 3** Opinion picker (role-branched via `ROLE_TO_BUCKET`) → **Screen 4** Audience+Voice. Progress dots, fade transition, Back/Next nav, per-screen validation. On "Generate my first post →": saves a partial profile via `POST /profile` (using `roleLabel` as the `name` field so `has_profile` becomes `true`), then shows the generating placeholder screen. |
...
| `/first-post` is a standalone conversion-focused route — no sidebar, no AppShell, handles its own auth check | Separate from `/onboarding` which remains the full 5-step profile flow for users who sign up directly. `/first-post` targets the acquisition funnel: unauthenticated visitors see a custom auth invite card (Google OAuth + email); authenticated users go straight into a 5-screen guided wizard (role → topic → experience → opinion → audience/voice) that collects just enough data to generate one post. All state is local React; no backend calls until generation (wired in a follow-up prompt). `/onboarding` is still the redirect target for new users who sign up via Clerk's sign-up page. |
