...
| `frontend/app/welcome/page.tsx` | Landing page (`/welcome`) — default entry point for unauthenticated visitors (middleware redirects `/` → `/welcome` when not signed in); editorial atelier marketing page; fixed glassmorphic nav with `font-headline italic` logo + CSS-only dropdowns; nav "Log in" → `/sign-in`, nav "Get started" + all hero/bottom CTAs ("Start writing for free") → `/sign-up`; hero with Noto Serif h1, feature cards, philosophy section, bento grid, `MOOD_GRADIENTS` staggered mood grid (8 tiles, charcode-deterministic), final CTA with dot-grid overlay; own top nav, AppShell sidebar suppressed |
...
| `frontend/middleware.ts` | Clerk v5 middleware — three routing rules: (1) unauthenticated visitors hitting `/` are redirected to `/welcome` (landing page as default entry point); (2) signed-in users hitting `/welcome` are redirected to `/` (no re-showing the landing page); (3) all other unauthenticated requests to protected routes (`/library`, `/create`, `/ideas`, `/history`, `/settings`, `/`) are redirected to `/sign-in`. Public routes that bypass all checks: `/welcome`, `/sign-in(.*)`, `/sign-up(.*)`, `/onboarding`. |
